...
class WPFG_Risk_Score {
    ...
    public static function calculate() {
        ...
        // 13. Security hardening? (-5 max)
        $factors['hardening'] = self::check_hardening();
        $score -= $factors['hardening']['deduction'];

        // 14. Firewall active? (-5)
        $factors['firewall'] = self::check_firewall();
        $score -= $factors['firewall']['deduction'];

        // 15. Two-Factor Auth? (-5)
        $factors['two_factor'] = self::check_two_factor();
        $score -= $factors['two_factor']['deduction'];

        // 16. Known vulnerabilities? (-15 max)
        $factors['vulnerabilities'] = self::check_vulnerabilities();
        $score -= $factors['vulnerabilities']['deduction'];

        // 17. File permissions? (-5 max)
        $factors['permissions'] = self::check_permissions();
        $score -= $factors['permissions']['deduction'];
    ...
    private static function check_hardening() {
        $count = 0;
        $total = 6;
        if ( WPFG_Settings::get( 'hardening_disable_file_editor', false ) ) $count++;
        if ( WPFG_Settings::get( 'hardening_disable_xmlrpc', false ) ) $count++;
        if ( WPFG_Settings::get( 'hardening_security_headers', true ) ) $count++;
        if ( WPFG_Settings::get( 'hardening_block_php_uploads', false ) ) $count++;
        if ( WPFG_Settings::get( 'hardening_disable_rest_unauth', false ) ) $count++;
        if ( WPFG_Settings::get( 'hardening_hide_wp_version', false ) ) $count++;

        if ( $count >= 4 ) {
            return array( 'label' => sprintf( __( '%d of %d hardening measures active', 'wp-file-guardian' ), $count, $total ), 'status' => 'good', 'deduction' => 0 );
        }
        if ( $count >= 2 ) {
            return array( 'label' => sprintf( __( '%d of %d hardening measures active', 'wp-file-guardian' ), $count, $total ), 'status' => 'ok', 'deduction' => 2 );
        }
        return array( 'label' => sprintf( __( 'Only %d of %d hardening measures active', 'wp-file-guardian' ), $count, $total ), 'status' => 'bad', 'deduction' => 5 );
    }

    private static function check_firewall() {
        if ( WPFG_Settings::get( 'firewall_enabled', false ) ) {
            return array( 'label' => __( 'Firewall is active', 'wp-file-guardian' ), 'status' => 'good', 'deduction' => 0 );
        }
        return array( 'label' => __( 'Firewall is not enabled', 'wp-file-guardian' ), 'status' => 'ok', 'deduction' => 5 );
    }

    private static function check_two_factor() {
        if ( ! WPFG_Settings::get( 'two_factor_enabled', false ) ) {
            return array( 'label' => __( 'Two-Factor Auth is not enabled', 'wp-file-guardian' ), 'status' => 'ok', 'deduction' => 5 );
        }
        // Check if admins have 2FA enabled.
        $admins = get_users( array( 'role' => 'administrator', 'fields' => 'ID' ) );
        $enabled = 0;
        foreach ( $admins as $uid ) {
            if ( get_user_meta( $uid, 'wpfg_2fa_enabled', true ) ) {
                $enabled++;
            }
        }
        if ( $enabled === count( $admins ) ) {
            return array( 'label' => __( 'All admins have 2FA enabled', 'wp-file-guardian' ), 'status' => 'good', 'deduction' => 0 );
        }
        return array( 'label' => sprintf( __( '%d of %d admins have 2FA', 'wp-file-guardian' ), $enabled, count( $admins ) ), 'status' => 'ok', 'deduction' => 3 );
    }
    ...
    private static function check_vulnerabilities() {
        ...
        if ( $wpdb->get_var( "SHOW TABLES LIKE '{$table}'" ) !== $table ) {
            return array( 'label' => __( 'Vulnerability scanner not configured', 'wp-file-guardian' ), 'status' => 'ok', 'deduction' => 5 );
        }
        $critical = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table} WHERE severity IN ('critical','high') AND status = 'open'" );
        if ( $critical > 0 ) {
            $deduction = min( 15, $critical * 5 );
            return array( 'label' => sprintf( __( '%d critical/high vulnerabilities found', 'wp-file-guardian' ), $critical ), 'status' => 'bad', 'deduction' => $deduction );
        }
        $open = (int) $wpdb->get_var( "SELECT COUNT(*) FROM {$table} WHERE status = 'open'" );
        if ( $open > 0 ) {
            return array( 'label' => sprintf( __( '%d open vulnerabilities (non-critical)', 'wp-file-guardian' ), $open ), 'status' => 'ok', 'deduction' => 3 );
        }
        return array( 'label' => __( 'No known vulnerabilities', 'wp-file-guardian' ), 'status' => 'good', 'deduction' => 0 );
    }

    private static function check_permissions() {
        $cached = get_transient( 'wpfg_permission_check_result' );
        if ( false !== $cached ) {
            return $cached;
        }
        // Default: not checked yet.
        $result = array( 'label' => __( 'File permissions not checked yet', 'wp-file-guardian' ), 'status' => 'ok', 'deduction' => 3 );
        set_transient( 'wpfg_permission_check_result', $result, 6 * HOUR_IN_SECONDS );
        return $result;
    }
    ...
    public static function get_recommendations() {
        ...
        // Sort by deduction descending (most impactful first).
    ...
    public static function get_score_history( $days = 30 ) {
        ...
        return $wpdb->get_results( $wpdb->prepare(
            "SELECT scan_date, risk_score FROM {$table} WHERE scan_date >= %s ORDER BY scan_date ASC",
            $since
        ) );
    }
}
