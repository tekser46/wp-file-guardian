...
class WPFG_File_Monitor {
    ...
    public static function run_check() {
        ...
        if ( $changes > 0 ) {
            ...
            // Email notification — always notify on any change.
            $subject = sprintf(
                __( 'File Changes Detected: %d added, %d modified, %d deleted', 'wp-file-guardian' ),
                $result['total_added'],
            ...
            // List changed file paths (max 20 each).
            if ( ! empty( $result['added'] ) ) {
                $body .= "\n\n" . __( '--- Added Files ---', 'wp-file-guardian' );
                foreach ( array_slice( $result['added'], 0, 20 ) as $f ) {
                    $body .= "\n" . $f['path'];
                }
                if ( $result['total_added'] > 20 ) {
                    $body .= sprintf( "\n... " . __( 'and %d more', 'wp-file-guardian' ), $result['total_added'] - 20 );
                }
            }
            if ( ! empty( $result['modified'] ) ) {
                $body .= "\n\n" . __( '--- Modified Files ---', 'wp-file-guardian' );
                foreach ( array_slice( $result['modified'], 0, 20 ) as $f ) {
                    $body .= "\n" . $f['path'];
                }
                if ( $result['total_modified'] > 20 ) {
                    $body .= sprintf( "\n... " . __( 'and %d more', 'wp-file-guardian' ), $result['total_modified'] - 20 );
                }
            }
            if ( ! empty( $result['deleted'] ) ) {
                $body .= "\n\n" . __( '--- Deleted Files ---', 'wp-file-guardian' );
                foreach ( array_slice( $result['deleted'], 0, 20 ) as $f ) {
                    $body .= "\n" . $f['path'];
                }
                if ( $result['total_deleted'] > 20 ) {
                    $body .= sprintf( "\n... " . __( 'and %d more', 'wp-file-guardian' ), $result['total_deleted'] - 20 );
                }
            }

            $body .= "\n\n" . sprintf( __( 'Review changes: %s', 'wp-file-guardian' ), admin_url( 'admin.php?page=wpfg-monitor' ) );
